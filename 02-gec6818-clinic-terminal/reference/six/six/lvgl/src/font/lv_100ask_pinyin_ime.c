/*
 * 文件名：lv_100ask_pinyin_ime.c
 * 版本说明：答辩版已修正项目常用景区候选字。
 *
 * 重要说明：
 * 1. 这是 LVGL 的 100ask 拼音输入法源码。
 * 2. 本文件不能随便打乱词库顺序，词库必须按 a、b、c……z 的拼音顺序排列。
 * 3. 之前把 shan、xi、huangshan 等条目整体插到 k 区域，会影响输入法索引。
 * 4. 本版本采用正确做法：只调整原有拼音条目，把项目常用字放到候选前面。
 * 5. 修改后需要 make clean 再重新编译，确保 lv_100ask_pinyin_ime.c 被重新编译。
 */
 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_100ask_pinyin_ime_class = {.constructor_cb = lv_100ask_pinyin_ime_constructor,
...
lv_100ask_pinyin_dict_t lv_100ask_def_pinyin_dict[] = {
    ...
    {"hong", "轰哄烘虹鸿洪宏弘红"},
    {"hou", "喉侯猴吼厚候后"},
    {"hu", "湖呼乎忽瑚壶葫胡蝴狐糊弧虎护互沪户唬和"},
    {"hua", "花华猾画化话哗滑划"},
    {"huai", "槐怀淮徊坏"},
    {"huan", "欢环桓缓换患唤痪豢焕涣宦幻还"},
    {"huang", "黄荒慌磺蝗簧皇凰惶煌晃幌恍谎"},
