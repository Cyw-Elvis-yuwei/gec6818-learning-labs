...
- 这是什么意思：本文是 `gec6818_profile.json` 的人类可读摘要，不是独立配置源。
...
机器可读事实以 [gec6818_profile.json](./gec6818_profile.json) 为唯一准则。本文的值若与 JSON 不一致，应先核对证据，再修正文档投影，不能反向用本文覆盖 JSON。

## 当前结论

| 范围 | 值 | 状态 | 证据 | 主要风险/验证 |
|---|---|---|---|---|
| 板卡名称 | GEC6818 | `confirmed-source` | 任务说明、当前 PRD | 名称不能证明 SoC/板卡修订；读 `/proc/cpuinfo` |
...
参考医疗后端“13 项主机测试通过”已移入 `evidence_sources`，作用域固定为 `host-backend`；它不属于 GEC6818 板级或工具链平台参数。当前板级 `confirmed-runtime` 数量为 0。
...
完整字段、逐项风险和验证命令见 JSON；关键未知项及升级流程见 [GEC6818_KNOWN_UNKNOWNS.md](./GEC6818_KNOWN_UNKNOWNS.md)。
