/*
 * 文件作用：Ubuntu 主机端“按 user_id 查询当天当前号单”客户端。
 * 它发送 get_current_ticket 并严格解析 Ticket 与 QueueSummary，用于验证当前叫号、
 * 前方等待人数和无号单错误；正式开发板由 ticket_client 执行同类请求。
 */
#ifndef _POSIX_C_SOURCE
#define _POSIX_C_SOURCE 200809L
#endif

#include "clinic_net.h"
#include "clinic_protocol.h"
#include "clinic_types.h"

#include <cjson/cJSON.h>

#include <errno.h>
#include <inttypes.h>
#include <limits.h>
#include <poll.h>
#include <signal.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>

#define CURRENT_TICKET_TIMEOUT_SECONDS 5
#define CURRENT_TICKET_NOT_FOUND_EXIT_STATUS 2
#define JSON_EXACT_INTEGER_MAX UINT64_C(9007199254740991)
#define TICKET_FIELD_COUNT 8U

typedef enum CurrentTicketResponseKind
{
    CURRENT_TICKET_RESPONSE_SUCCESS = 0,
    CURRENT_TICKET_RESPONSE_NOT_FOUND,
    CURRENT_TICKET_RESPONSE_SERVER_ERROR
} CurrentTicketResponseKind;

static int parse_uint64_text(const char *text, uint64_t *value)
{
    const char *cursor;
    uint64_t parsed = 0U;

    if (text == NULL || value == NULL || text[0] == '\0')
    {
        return -1;
    }
    for (cursor = text; *cursor != '\0'; ++cursor)
    {
        uint64_t digit;

        if (*cursor < '0' || *cursor > '9')
        {
            return -1;
        }
        digit = (uint64_t)(unsigned char)(*cursor - '0');
        if (parsed > (UINT64_MAX - digit) / UINT64_C(10))
        {
            return -1;
        }
        parsed = parsed * UINT64_C(10) + digit;
    }
    *value = parsed;
    return 0;
}

static int parse_positive_safe_int64_text(const char *text, int64_t *value)
{
    uint64_t parsed;

    if (value == NULL || parse_uint64_text(text, &parsed) != 0 ||
        parsed == 0U || parsed > JSON_EXACT_INTEGER_MAX)
    {
        return -1;
    }
    *value = (int64_t)parsed;
    return 0;
}

static int port_is_valid(const char *text)
{
    uint64_t port;

    return parse_uint64_text(text, &port) == 0 &&
        port > 0U && port <= UINT64_C(65535);
}

static char *create_request(
    uint64_t request_id,
    int64_t user_id,
    size_t *request_length)
{
    char request_id_text[32];
    char user_id_text[32];
    cJSON *root = NULL;
    char *json = NULL;
    char *request = NULL;
    size_t json_length;
    int written;

    if (request_length == NULL)
    {
        return NULL;
    }
    *request_length = 0U;
    written = snprintf(
        request_id_text,
        sizeof(request_id_text),
        "%" PRIu64,
        request_id);
    if (written < 0 || (size_t)written >= sizeof(request_id_text))
    {
        return NULL;
    }
    written = snprintf(
        user_id_text,
        sizeof(user_id_text),
        "%" PRId64,
        user_id);
    if (written < 0 || (size_t)written >= sizeof(user_id_text))
    {
        return NULL;
    }

    root = cJSON_CreateObject();
    if (root == NULL ||
        cJSON_AddStringToObject(root, "type", "get_current_ticket") == NULL ||
        cJSON_AddRawToObject(root, "request_id", request_id_text) == NULL ||
        cJSON_AddRawToObject(root, "user_id", user_id_text) == NULL)
    {
        cJSON_Delete(root);
        return NULL;
    }
    json = cJSON_PrintUnformatted(root);
    cJSON_Delete(root);
    if (json == NULL)
    {
        return NULL;
    }

    json_length = strlen(json);
    if (json_length <= CLINIC_MAX_FRAME_SIZE)
    {
        request = malloc(json_length + 2U);
    }
    if (request != NULL)
    {
        memcpy(request, json, json_length);
        request[json_length] = '\n';
        request[json_length + 1U] = '\0';
        *request_length = json_length + 1U;
    }
    cJSON_free(json);
    return request;
}

static int receive_response(
    clinic_socket_t socket_fd,
    char *response,
    size_t response_capacity,
    size_t *response_length)
{
    size_t received_total = 0U;
    struct timespec deadline;

    if (response == NULL || response_length == NULL ||
        response_capacity < CLINIC_MAX_FRAME_SIZE + 1U)
    {
        return -1;
    }
    *response_length = 0U;
    response[0] = '\0';
    if (clock_gettime(CLOCK_MONOTONIC, &deadline) != 0)
    {
        return -1;
    }
    deadline.tv_sec += CURRENT_TICKET_TIMEOUT_SECONDS;

    while (received_total < CLINIC_MAX_FRAME_SIZE)
    {
        struct pollfd descriptor;
        int poll_status;

        descriptor.fd = socket_fd;
        descriptor.events = POLLIN;
        descriptor.revents = 0;
        for (;;)
        {
            struct timespec now;
            int64_t remaining_seconds;
            long remaining_nanoseconds;
            int64_t timeout_milliseconds;

            if (clock_gettime(CLOCK_MONOTONIC, &now) != 0)
            {
                response[0] = '\0';
                return -1;
            }
            remaining_seconds =
                (int64_t)deadline.tv_sec - (int64_t)now.tv_sec;
            remaining_nanoseconds = deadline.tv_nsec - now.tv_nsec;
            if (remaining_nanoseconds < 0L)
            {
                --remaining_seconds;
                remaining_nanoseconds += 1000000000L;
            }
            if (remaining_seconds < 0)
            {
                response[0] = '\0';
                return -1;
            }
            timeout_milliseconds =
                remaining_seconds * INT64_C(1000) +
                (remaining_nanoseconds + 999999L) / 1000000L;
            if (timeout_milliseconds <= 0)
            {
                response[0] = '\0';
                return -1;
            }
            if (timeout_milliseconds > INT_MAX)
            {
                timeout_milliseconds = INT_MAX;
            }
            poll_status = poll(&descriptor, 1U, (int)timeout_milliseconds);
            if (poll_status >= 0 || errno != EINTR)
            {
                break;
            }
        }
        if (poll_status <= 0 ||
            (descriptor.revents & (POLLIN | POLLHUP)) == 0)
        {
            response[0] = '\0';
            return -1;
        }

        {
            int received = recv(
                socket_fd,
                response + received_total,
                (int)(CLINIC_MAX_FRAME_SIZE - received_total),
                0);
            char *newline;

            if (received <= 0)
            {
                response[0] = '\0';
                return -1;
            }
            received_total += (size_t)received;
            response[received_total] = '\0';
            newline = memchr(response, '\n', received_total);
            if (newline != NULL)
            {
                *response_length = (size_t)(newline - response) + 1U;
                response[*response_length] = '\0';
                return 0;
            }
        }
    }
    response[0] = '\0';
    return -1;
}

static cJSON *get_unique_object_item(const cJSON *object, const char *name)
{
    cJSON *child;
    cJSON *match = NULL;
    unsigned int matches = 0U;

    if (!cJSON_IsObject(object) || name == NULL)
    {
        return NULL;
    }
    for (child = object->child; child != NULL; child = child->next)
    {
        if (child->string != NULL && strcmp(child->string, name) == 0)
        {
            match = child;
            ++matches;
        }
    }
    return matches == 1U ? match : NULL;
}

static int object_has_exact_fields(
    const cJSON *object,
    const char *const *field_names,
    size_t field_count)
{
    unsigned int seen[TICKET_FIELD_COUNT] = {0U};
    const cJSON *child;
    size_t total = 0U;
    size_t index;

    if (!cJSON_IsObject(object) || field_names == NULL ||
        field_count > TICKET_FIELD_COUNT)
    {
        return 0;
    }
    for (child = object->child; child != NULL; child = child->next)
    {
        int matched = 0;

        if (child->string == NULL)
        {
            return 0;
        }
        for (index = 0U; index < field_count; ++index)
        {
            if (strcmp(child->string, field_names[index]) == 0)
            {
                ++seen[index];
                matched = 1;
                break;
            }
        }
        if (!matched || seen[index] > 1U)
        {
            return 0;
        }
        ++total;
    }
    if (total != field_count)
    {
        return 0;
    }
    for (index = 0U; index < field_count; ++index)
    {
        if (seen[index] != 1U)
        {
            return 0;
        }
    }
    return 1;
}

static int json_number_is_positive_safe_integer(const cJSON *item)
{
    uint64_t value;

    if (!cJSON_IsNumber(item) || item->valuedouble < 1.0 ||
        item->valuedouble > (double)JSON_EXACT_INTEGER_MAX)
    {
        return 0;
    }
    value = (uint64_t)item->valuedouble;
    return (double)value == item->valuedouble;
}

static int json_number_is_nonnegative_safe_integer(const cJSON *item)
{
    uint64_t value;

    if (!cJSON_IsNumber(item) || item->valuedouble < 0.0 ||
        item->valuedouble > (double)JSON_EXACT_INTEGER_MAX)
    {
        return 0;
    }
    value = (uint64_t)item->valuedouble;
    return (double)value == item->valuedouble;
}

static int status_is_known(const cJSON *status)
{
    return cJSON_IsString(status) && status->valuestring != NULL &&
        (strcmp(status->valuestring, "WAITING") == 0 ||
         strcmp(status->valuestring, "CALLED") == 0 ||
         strcmp(status->valuestring, "COMPLETED") == 0 ||
         strcmp(status->valuestring, "CANCELLED") == 0);
}

static int ticket_is_valid(const cJSON *ticket, int64_t expected_user_id)
{
    static const char *const ticket_fields[] = {
        "id",
        "user_id",
        "department_id",
        "queue_number",
        "status",
        "service_date",
        "created_time",
        "called_time"
    };
    cJSON *id;
    cJSON *user_id;
    cJSON *department_id;
    cJSON *queue_number;
    cJSON *status;
    cJSON *service_date;
    cJSON *created_time;
    cJSON *called_time;

    if (!object_has_exact_fields(
            ticket,
            ticket_fields,
            sizeof(ticket_fields) / sizeof(ticket_fields[0])))
    {
        return 0;
    }
    id = get_unique_object_item(ticket, "id");
    user_id = get_unique_object_item(ticket, "user_id");
    department_id = get_unique_object_item(ticket, "department_id");
    queue_number = get_unique_object_item(ticket, "queue_number");
    status = get_unique_object_item(ticket, "status");
    service_date = get_unique_object_item(ticket, "service_date");
    created_time = get_unique_object_item(ticket, "created_time");
    called_time = get_unique_object_item(ticket, "called_time");

    return json_number_is_positive_safe_integer(id) &&
        json_number_is_positive_safe_integer(user_id) &&
        user_id->valuedouble == (double)expected_user_id &&
        json_number_is_positive_safe_integer(department_id) &&
        json_number_is_positive_safe_integer(queue_number) &&
        status_is_known(status) &&
        cJSON_IsString(service_date) && service_date->valuestring != NULL &&
        strlen(service_date->valuestring) == CLINIC_SERVICE_DATE_LENGTH &&
        json_number_is_positive_safe_integer(created_time) &&
        (cJSON_IsNull(called_time) ||
         json_number_is_positive_safe_integer(called_time));
}

static int queue_summary_is_valid(const cJSON *summary)
{
    static const char *const summary_fields[] = {
        "current_called_queue_number",
        "waiting_ahead_count"
    };
    cJSON *current_called;
    cJSON *waiting_ahead;

    if (!object_has_exact_fields(
            summary,
            summary_fields,
            sizeof(summary_fields) / sizeof(summary_fields[0])))
    {
        return 0;
    }
    current_called = get_unique_object_item(
        summary,
        "current_called_queue_number");
    waiting_ahead = get_unique_object_item(summary, "waiting_ahead_count");
    return (cJSON_IsNull(current_called) ||
            json_number_is_positive_safe_integer(current_called)) &&
        json_number_is_nonnegative_safe_integer(waiting_ahead);
}

static int validate_response(
    const char *response,
    size_t response_length,
    uint64_t expected_request_id,
    int64_t expected_user_id,
    CurrentTicketResponseKind *response_kind)
{
    static const char *const success_fields[] = {
        "ok", "request_id", "ticket", "queue_summary", "message"
    };
    static const char *const error_fields[] = {
        "ok", "request_id", "error_code", "message"
    };
    const char *parse_end = NULL;
    cJSON *root;
    cJSON *ok;
    cJSON *request_id;
    cJSON *message;

    if (response == NULL || response_kind == NULL || response_length == 0U ||
        response[response_length - 1U] != '\n' ||
        memchr(response, '\n', response_length - 1U) != NULL)
    {
        return -1;
    }
    root = cJSON_ParseWithOpts(response, &parse_end, 1);
    if (!cJSON_IsObject(root) || parse_end != response + response_length)
    {
        cJSON_Delete(root);
        return -1;
    }

    ok = get_unique_object_item(root, "ok");
    request_id = get_unique_object_item(root, "request_id");
    message = get_unique_object_item(root, "message");
    if ((!cJSON_IsTrue(ok) && !cJSON_IsFalse(ok)) ||
        !json_number_is_positive_safe_integer(request_id) ||
        request_id->valuedouble != (double)expected_request_id ||
        !cJSON_IsString(message) || message->valuestring == NULL)
    {
        cJSON_Delete(root);
        return -1;
    }

    if (cJSON_IsTrue(ok))
    {
        cJSON *ticket = get_unique_object_item(root, "ticket");

        if (!object_has_exact_fields(
                root,
                success_fields,
                sizeof(success_fields) / sizeof(success_fields[0])) ||
            !ticket_is_valid(ticket, expected_user_id) ||
            !queue_summary_is_valid(
                get_unique_object_item(root, "queue_summary")))
        {
            cJSON_Delete(root);
            return -1;
        }
        *response_kind = CURRENT_TICKET_RESPONSE_SUCCESS;
    }
    else
    {
        cJSON *error_code = get_unique_object_item(root, "error_code");

        if (!object_has_exact_fields(
                root,
                error_fields,
                sizeof(error_fields) / sizeof(error_fields[0])) ||
            !cJSON_IsString(error_code) || error_code->valuestring == NULL ||
            error_code->valuestring[0] == '\0')
        {
            cJSON_Delete(root);
            return -1;
        }
        *response_kind = strcmp(
            error_code->valuestring,
            "CURRENT_TICKET_NOT_FOUND") == 0
            ? CURRENT_TICKET_RESPONSE_NOT_FOUND
            : CURRENT_TICKET_RESPONSE_SERVER_ERROR;
    }

    cJSON_Delete(root);
    return 0;
}

int main(int argc, char **argv)
{
    const char *host;
    const char *port;
    uint64_t request_id;
    int64_t user_id;
    clinic_socket_t socket_fd = CLINIC_SOCKET_INVALID;
    char *request;
    char response[CLINIC_MAX_FRAME_SIZE + 1U];
    size_t request_length = 0U;
    size_t response_length = 0U;
    CurrentTicketResponseKind response_kind =
        CURRENT_TICKET_RESPONSE_SERVER_ERROR;
    int exit_status = EXIT_FAILURE;
    int network_started = 0;

    if (argc != 5)
    {
        fprintf(
            stderr,
            "usage: %s <server_ip> <port> <request_id> <user_id>\n",
            argv[0]);
        return EXIT_FAILURE;
    }
    host = argv[1];
    port = argv[2];
    if (host[0] == '\0' || !port_is_valid(port) ||
        parse_uint64_text(argv[3], &request_id) != 0 || request_id == 0U ||
        request_id > JSON_EXACT_INTEGER_MAX ||
        parse_positive_safe_int64_text(argv[4], &user_id) != 0)
    {
        fprintf(stderr, "invalid current ticket arguments\n");
        return EXIT_FAILURE;
    }

    request = create_request(request_id, user_id, &request_length);
    if (request == NULL)
    {
        fprintf(stderr, "could not encode current ticket request\n");
        return EXIT_FAILURE;
    }

    (void)signal(SIGPIPE, SIG_IGN);
    if (clinic_net_startup() != 0)
    {
        fprintf(stderr, "network startup failed\n");
        free(request);
        return EXIT_FAILURE;
    }
    network_started = 1;
    if (clinic_net_connect(host, port, &socket_fd) != 0)
    {
        fprintf(stderr, "could not connect to %s:%s\n", host, port);
        goto cleanup;
    }
    if (clinic_net_send_all(socket_fd, request, request_length) != 0 ||
        receive_response(
            socket_fd,
            response,
            sizeof(response),
            &response_length) != 0 ||
        validate_response(
            response,
            response_length,
            request_id,
            user_id,
            &response_kind) != 0)
    {
        fprintf(stderr, "current ticket request failed\n");
        goto cleanup;
    }
    if (fwrite(response, 1U, response_length, stdout) != response_length)
    {
        fprintf(stderr, "could not write current ticket response\n");
        goto cleanup;
    }

    if (response_kind == CURRENT_TICKET_RESPONSE_SUCCESS)
    {
        exit_status = EXIT_SUCCESS;
    }
    else if (response_kind == CURRENT_TICKET_RESPONSE_NOT_FOUND)
    {
        exit_status = CURRENT_TICKET_NOT_FOUND_EXIT_STATUS;
    }
    else
    {
        fprintf(stderr, "current ticket request returned a server error\n");
    }

cleanup:
    if (socket_fd != CLINIC_SOCKET_INVALID)
    {
        clinic_socket_close(socket_fd);
    }
    if (network_started)
    {
        clinic_net_cleanup();
    }
    free(request);
    return exit_status;
}
