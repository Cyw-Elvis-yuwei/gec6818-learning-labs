...
- 这是什么意思：既有用户构建和 S5P6818 功能确认已按证据边界归档；本轮只同步材料，不产生新的功能验证结果。
...
## L. S5P6818 最小 LVGL framebuffer 显示验证
...
- LCD 正确显示纯色背景和居中的 `S5P6818 LVGL OK`。
- 未出现花屏、偏移、裁切或崩溃。
- Ctrl+C 正常停止。
- 退出状态：`run_exit=0`。
...
- 主机端完整业务闭环：完成。
- S5P6818 工具链：完成。
- S5P6818 最小 LVGL framebuffer 显示：完成。
- S5P6818 最小 LVGL 触摸输入：完成。
- S5P6818 LVGL 中文字体最小验证：完成。
- S5P6818 板端最小 TCP 网络：完成。
- S5P6818 LVGL 后台网络线程：完成。
- LVGL 医疗登录页骨架：完成。
- 登录页之外的医疗业务页面：未开始。
- 下一唯一阶段：板端真实 login TCP 请求。

## M. S5P6818 最小 LVGL 触摸验证
...
- 能够证明：登录页输入、键盘切换、模态校验提示和退出流程在 S5P6818 上可用。
...
- LVGL 医疗登录页骨架：当前阶段完成。
- 板端真实 login TCP 请求：当前阶段完成，已由用户在 S5P6818 实机验证。
...
## S. S5P6818 LVGL 医疗服务主页骨架实机验证

| 项目 | 记录 |
| --- | --- |
| 执行者 | 用户 |
| 环境 | S5P6818 开发板 / Ubuntu 真实登录服务器 |
| 对应阶段 | 登录成功后的医疗服务主页骨架 |
| 结果 | 用户确认按本阶段验收范围通过 |
...
- 能够证明：登录成功消息框确认后的页面切换、真实 `user_id` 从登录结果传递到主页并显示、四个入口当时的占位交互、登录页对象清理、UI 响应和退出流程在 S5P6818 上正常。
- 本节当时不能证明科室、医生、取号或排队的真实请求；后续科室列表验证证据见 T 节。
- 当前阶段：登录成功后的医疗服务主页骨架，当前阶段完成。
- 后续科室列表真实请求与展示阶段也已完成，见 T 节。

## T. S5P6818 科室列表真实请求与展示实机验证

| 项目 | 记录 |
| --- | --- |
| 执行者 | 用户 |
| 环境 | S5P6818 开发板 / Ubuntu `clinic_server` / SQLite |
| 对应阶段 | 科室列表真实请求与展示 |
| 结果 | 用户确认按本阶段验收范围通过 |
...
- SQLite 返回的真实科室名称和 ID 已在 S5P6818 科室页面成功展示。
...
- SQLite 返回的该科室真实医生数据已在 S5P6818 医生页面正确展示。
...
- SQLite 成功创建真实号单，返回的真实号单数据已在 S5P6818 正确展示。
...
- 本节当时不能证明 S5P6818 板端当前号单查询与排队页面；后续用户实机证据见 X 节。
- 排队状态自动轮询。
...
- S5P6818 已完成真实 `get_current_ticket` TCP 请求、`clinic_server` / SQLite 查询和 LVGL 排队状态展示闭环。
...
- 板端 `CURRENT_TICKET_NOT_FOUND` 无号单分支；该分支只有主机端历史证据，本轮未在 S5P6818 触发。
...
| --- | --- |
| 执行者 | 用户 |
| 环境 | Ubuntu `clinic_server` / SQLite / S5P6818 开发板 |
...
| 板端当前号单无号单分支 | 未验证 | S5P6818 实机触发 `CURRENT_TICKET_NOT_FOUND` 的证据 |
| 板端其余号单状态 | 未验证 | S5P6818 实机分别展示 `COMPLETED` 和 `CANCELLED` 的证据；`CALLED` 已在 Y 节验证 |
| 排队状态自动轮询与服务器推送 | 未验证 | 自动轮询或服务器主动推送的实现及实机证据 |
| 最终演示验收与项目完成边界审查 | 未验证 | 完成最终演示主链，并审查项目完成声明所需证据与未覆盖边界 |

## AB. S5P6818 注册登录认证链路实机确认

### 用户确认范围

- 用户已确认 S5P6818 开发板上的注册、注册成功返回登录页、登录和进入医疗服务主页流程成功。
...
- 下一唯一阶段：排队信息增强主机回归与 S5P6818 实机验收。
...
## AD. S5P6818 排队信息增强实机确认

| 项目 | 记录 |
| --- | --- |
| 执行者 | 用户 |
| 环境 | S5P6818 开发板 / Ubuntu 服务器 / SQLite |
...
本项目当前已完成本阶段计划范围内的主机回归、板端构建、部署和 S5P6818 实机验收。未将自动轮询、服务器推送、复杂预约和未实际触发的边界状态写成已验证。
...
| --- | --- |
| 执行者 | 用户 |
| 环境 | Ubuntu ARM 构建 / PowerShell 部署 / S5P6818开发板 |
| 构建 | 用户确认成功 |
| 部署与运行 | 用户确认成功 |
| 记住状态切换 | 用户确认通过 |
| 退出登录后回填 | 用户确认通过 |
...
- 登录页记住密码功能已有用户真实构建、部署和S5P6818实机确认。
...
用户验证总错误标志：`1`（尚缺 Ubuntu ARM 构建、部署和 S5P6818 实机证据）。
...
- 未证明本次最新源码已经在 S5P6818 上启动、点击或完成断网恢复；不把构建证据扩展为实机通过。
- 文档阶段完成，不改变已有用户验证结论。
...
| --- | --- |
| 执行者 | 用户 |
| 环境 | Windows PowerShell 部署、Ubuntu 服务器、S5P6818 开发板 |
...
- 本记录不扩展为 S5P6818 实机运行通过；开发板部署和最终演示仍是下一阶段。
- 文档阶段完成，不改变已有用户验证结论。
...
- 本记录不得解释为回归通过、ARM 构建通过或 S5P6818 实机通过。
...
- 本记录不扩展为 S5P6818 实机验证，也不改变原四参数 CLI 的既有验证结论。
...
| --- | --- |
| 执行者 | 用户 |
| 环境 | VMware Ubuntu 20.04、共享目录 `/mnt/hgfs/codex`、S5P6818 |
