...
- 项目名称：医路通
- 项目全称：基于 S5P6818 的联网智慧医疗自助服务终端
- 更新时间：2026-07-24 20:17（Asia/Hong_Kong）
- 状态用途：供后续 Codex 任务直接恢复项目进度，不以旧聊天记录作为事实来源。

## 当前演示目标

目标是在 S5P6818 终端与 Ubuntu 服务器之间形成可演示的医疗自助服务闭环：用户注册或登录，浏览科室和医生，按科室取号，查询排队状态，管理员叫号后由终端刷新结果。
...
| S5P6818 实机环境探针 | 当前阶段完成 | 已确认板端架构、内核、glibc、动态加载器、framebuffer、触摸节点、字体路径和 IP |
| 最终交叉工具链确认 | 当前阶段完成 | 已选定 ARM GCC 5.4.0 工具链，并由板端 ABI 探针验证产物可运行 |
| S5P6818 最小 LVGL framebuffer 显示 | 当前阶段完成 | LVGL 8.3.0 程序已交叉构建并通过 `/dev/fb0` 实机显示与退出验证 |
| S5P6818 最小 LVGL 触摸输入 | 当前阶段完成 | `gslX680` 经 `/dev/input/event0`、evdev 和 LVGL pointer input 触发 `LV_EVENT_CLICKED`，实机点击验证通过 |
| S5P6818 LVGL 中文字体最小验证 | 当前阶段完成 | `/font/simkai.ttf` 经 LVGL 8.3 FreeType 支持清晰显示中文，并与 framebuffer、触摸输入同时稳定工作 |
| S5P6818 板端最小 TCP 网络 | 当前阶段完成 | ARM 网络探针已连接 Ubuntu epoll `clinic_server`，完成换行 ping/pong JSON 并以 `probe_exit=0` 退出 |
| S5P6818 LVGL 后台网络线程 | 当前阶段完成 | LVGL 主线程、后台 pthread、互斥状态交换和线程回收已通过成功、失败、连续重试及退出实机验证 |
| S5P6818 LVGL 医疗登录页骨架 | 当前阶段完成 | 用户已在 S5P6818 实机确认中文表单、双键盘切换、本地校验模态提示和退出流程按既定标准通过 |
| 板端真实 login TCP 请求 | 当前阶段完成 | 用户已在 S5P6818 实机验证真实 login 请求、错误密码拒绝、正确密码成功、离线恢复重试和正常退出 |
| S5P6818 注册 → 返回登录 → 登录 → 主页认证链路 | 当前阶段完成 | 用户已在 S5P6818 实机确认注册成功返回登录、用户名回填、密码清空、登录成功进入主页及长输入和字段切换 |
| clinic_terminal 板端构建与 cJSON 依赖 | 当前阶段完成 | 直接编译项目内 cJSON 1.7.19；产物为 ARM EABI5，使用 `/IOT` RPATH 和 pthread，不依赖外部 libcjson |
| S5P6818 LVGL 医疗服务主页骨架 | 当前阶段完成 | 用户已在 S5P6818 实机确认登录成功后进入主页、显示真实 `user_id=1`、四个入口当时的占位交互可用且登录页对象无残留 |
| S5P6818 三种服务入口语义拆分 | 局部验证 | 科室查询、医生查询、门诊取号使用显式流程模式；用户反馈最新统一产物演示检查通过，但未保留逐项 S5P6818 点击日志，不升级为完整专项证据 |
| S5P6818 科室列表真实请求与展示 | 当前阶段完成 | 用户已在 S5P6818 实机确认真实 `list_departments` 请求、SQLite 科室名称与 ID 展示、离线恢复、返回主页和正常退出 |
| S5P6818 医生列表真实请求与展示 | 当前阶段完成 | 用户已在 S5P6818 实机确认真实 `department_id` 请求、SQLite 医生数据展示、离线恢复、缓存返回科室页和正常退出 |
| S5P6818 医生选择后的科室真实取号 | 当前阶段完成 | 用户已在 S5P6818 实机确认真实 `create_ticket` 请求、SQLite 号单写入、真实号单展示、离线恢复和返回主页 |
| S5P6818 板端当前号单查询与排队状态展示 | 当前阶段完成 | 用户已在 S5P6818 实机确认真实 `get_current_ticket` 请求、`WAITING` 中文展示、离线恢复、返回主页和正常退出 |
| S5P6818 排队状态手动刷新与叫号结果展示 | 当前阶段完成 | 用户已在 S5P6818 实机确认手动刷新、管理员 `call_next(department_id)` 后的 `CALLED` 中文展示、真实 `called_time`、离线恢复和正常退出 |
| S5P6818 排队信息增强：当前叫号与前方 `WAITING` 人数 | 当前阶段完成 | 用户确认部署运行后当前叫号、前方人数、手动刷新和人数每次减少 1 的本轮检查全部通过 |
| S5P6818 排队页 5 秒自动刷新 | 当前阶段完成 | 用户明确反馈“验证也通过了，自动轮询”；未保留本轮构建命令、退出码和逐项板端日志 |
| 排队页定时器生命周期封装 | 当前阶段完成 | `queue_page.c` 仅集中定时器暂停、重置恢复和销毁路径；Ubuntu/ARM 构建回传 `clean=0`、`board_build=0`、`artifact=0`，用户随后确认 S5P6818 实机生命周期回归通过；未保留原始逐项日志 |
...
| S5P6818 最新统一产物部署与最终演示流程 | 当前阶段完成 | 用户反馈部署、启动、三种服务入口、门诊取号、排队刷新、叫号、异常恢复和退出检查通过；未保留具体上传与叫号退出码 |
...
本阶段不运行构建、测试、程序或设备操作；既有用户回传的 `clean=0`、`board_build=0`、`artifact=0` 和 S5P6818 生命周期确认只作为已有验证事实引用，不重新解释为本阶段新验证。
...
## S5P6818 最小 LVGL framebuffer 显示阶段事实
...
- 部署链路：Ubuntu 交叉编译 → VMware 共享目录 → Windows PowerShell 使用 `scp -O` → S5P6818 `/IOT/clinic_lvgl_smoke`。
- LCD 纯色背景显示正确，屏幕中央显示 `S5P6818 LVGL OK`。
- 未出现花屏、偏移、裁切或崩溃。
- Ctrl+C 可以正常停止程序。
- 退出状态：`run_exit=0`。

## S5P6818 最小 LVGL 触摸验证阶段事实
...
- S5P6818 地址：`192.168.10.42`。
- TCP 端口：`9000`。
- Ubuntu 与 S5P6818 双向 ping 已通过，丢包率为 0%。

### 实际验证链路

S5P6818 → TCP → Ubuntu epoll `clinic_server` → ping JSON → pong JSON。
...
## S5P6818 LVGL 医疗登录页骨架阶段事实

### 已完成能力

- `board/clinic_terminal/` 已形成可在 S5P6818 上运行的 LVGL 医疗登录页骨架。
...
- 用户已在 S5P6818 开发板确认用户名 K26 拼音键盘和密码英文键盘可见、可用。
...
- LVGL 医疗登录页骨架：当前阶段完成。
- 真实 login TCP 请求：当前阶段完成，已由用户在 S5P6818 实机验证。
- 登录成功后的 `user_id` 保存与主页传递：当前阶段完成；用户已在主页实机确认显示服务器返回的 `user_id=1`。
- 登录成功页面跳转和医疗服务主页骨架：当前阶段完成。
- 注册页面：未开始。
- 科室列表真实请求与展示：当前阶段完成，已由用户在 S5P6818 实机验证。
- 医生列表真实请求与展示：当前阶段完成，已由用户在 S5P6818 实机验证。
- 医生选择后的科室真实取号：当前阶段完成，已由用户在 S5P6818 实机验证。
- 当前号单查询与排队状态展示：当前阶段完成，已由用户在 S5P6818 实机验证。

## S5P6818 LVGL 医疗服务主页骨架阶段事实
...
- 用户已执行主机回归、板端构建部署和 S5P6818 实机验收，并确认本轮检查全部通过。
...
| 模块 | 状态 | 边界 |
| --- | --- | --- |
| S5P6818 主页独立医生查询入口 | 局部验证 | 用户反馈最新统一产物演示检查通过；当前记录未保留逐项 S5P6818 点击日志，因此保留局部验证边界 |
| 排队页定时器生命周期封装 | 当前阶段完成 | Ubuntu/ARM 构建与产物存在性检查通过；用户已确认本次源码在 S5P6818 上完成生命周期回归，原始逐项日志未保留 |
| 项目完成边界审查与最终材料收口 | 当前阶段完成 | 已同步最终验收边界、演示顺序、证据完整性说明和未覆盖边界；等待用户确认当前范围是否收口 |

## 当前未完成边界

- S5P6818 实机环境探针、最终工具链确认、最小 LVGL framebuffer 显示、最小 LVGL 触摸输入、LVGL 中文字体最小验证、板端最小 TCP 网络、LVGL 后台网络线程、LVGL 医疗登录页骨架、医疗服务主页骨架、科室列表、医生列表、科室真实取号、当前号单查询与排队状态展示以及手动刷新与 `CALLED` 结果展示已经完成。
- VMware Ubuntu 继续作为编译环境，实际运行证据来自 IP 为 `192.168.10.42` 的 S5P6818 开发板。
...
- 本次定时器生命周期封装已取得 Ubuntu/ARM 构建和产物存在性证据；用户已确认 S5P6818 的反复进出页面、自动/手动刷新交互、断网恢复和定时器销毁回归通过，但未提供原始逐项日志。
...
“当前阶段完成”只表示指定范围已经收口。主机端完整业务闭环、主机端最终回归、`get_current_ticket` 后端协议与主机 TCP 闭环、S5P6818 工具链、基础 LVGL 能力、登录、主页、科室、医生、科室真实取号、板端当前号单查询、手动刷新与 `CALLED` 结果展示、统一构建入口以及本次最新产物演示流程均已有对应证据；项目整体仍等待用户确认最终验收边界，不扩展为原始 PRD 全部完成。
...
- 本轮用户构建结果：`clean=0`、`board_build=0`、`artifact=0`；用户随后明确反馈“验证通过”，完成本次 S5P6818 生命周期回归确认，但未保留原始逐项日志。
...
## AU. 排队页定时器生命周期封装 S5P6818 实机回归确认

更新时间：2026-07-24 20:10（Asia/Hong_Kong）

### V-AU-01：S5P6818 实机生命周期回归
...
- 本轮修改尚未在 Ubuntu 重新交叉构建，尚未在 S5P6818 实机验证。
...
- 尚未在 Ubuntu 重新 ARM 交叉编译，尚未在 S5P6818 实机验证。
...
- 该证据不证明开发板已运行本次最新源码，也不替代 S5P6818 实机点击、断网恢复和完整演示验收。
- 文档阶段完成，不改变已有用户验证结论。
...
- 既有 S5P6818 演示通过证据早于本轮修改，不能替代本轮重复取号提示的实机验证。
...
- 未修改 Handler、Core、Store、SQLite、叫号顺序和 S5P6818 板端功能。
- 未在普通用户板端增加叫号权限。
...
- 本次确认只覆盖 Ubuntu 管理端交互式叫号，不新增 S5P6818 实机结论。
...
| 不能证明 | S5P6818 部署、启动、自动/手动刷新、反复进出页面、断网恢复或定时器销毁行为 |

### 阶段结论

- 本阶段状态：局部验证。
- 用户验证错误标志：`0`。
- 下一唯一阶段：S5P6818 排队页定时器生命周期回归。
