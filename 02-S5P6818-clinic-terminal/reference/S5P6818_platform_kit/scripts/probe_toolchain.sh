...
# Inspect only the three documented S5P6818 compiler candidates.  This script
...
Inspect the three documented S5P6818 compiler candidates.  For each available
...
create_temp_source() {
    ...
    if have_command mktemp; then
        TEMP_DIR=$(mktemp -d /tmp/s5p6818-toolchain-probe.XXXXXX 2>/dev/null || true)
    fi
    if [ -z "$TEMP_DIR" ]; then
        TEMP_DIR=/tmp/s5p6818-toolchain-probe.$$
...
int s5p6818_toolchain_probe_add(int left, int right)
{
    return left + right;
}
EOF
