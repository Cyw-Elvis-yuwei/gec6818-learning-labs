...
{
    cat <<'EOF'
Usage: bootstrap_s5p6818_lvgl8_project.sh [--allow-unverified-defaults] <new-directory>
...
PROFILE=$KIT_DIR/s5p6818_profile.json
...
STAGE=$TARGET_PARENT_ABS/.${TARGET_BASENAME}.s5p6818-bootstrap.$$
if [ -e "$STAGE" ]; then
...
if ! cp "$PROFILE" "$STAGE/s5p6818_profile.json" || \
...
if ! "$PYTHON" - "$STAGE/s5p6818_profile.json" "$STAGE" "$ALLOW_UNVERIFIED" <<'PY'
import json
import os
import re
import sys
...
header = """/* Generated from s5p6818_profile.json. */
/* DO NOT treat source defaults as live-board facts. */
#ifndef S5P6818_PLATFORM_CONFIG_H
#define S5P6818_PLATFORM_CONFIG_H

#define S5P6818_PROFILE_RUNTIME_CONFIRMED {runtime_confirmed}
#define S5P6818_PROFILE_SCHEMA_VERSION {schema_version}
#define S5P6818_UNVERIFIED_DEFAULTS_ALLOWED {allow_unverified}
#define S5P6818_UNVERIFIED_VALUES_PRESENT {unverified_present}
#define S5P6818_LVGL_API_MAJOR 8
#define S5P6818_LVGL_RECOMMENDED_VERSION "8.3.0"

#define S5P6818_FBDEV_PATH {fb_device} /* {fb_device_source} */
#define S5P6818_DISPLAY_WIDTH {width} /* {width_source} */
#define S5P6818_DISPLAY_HEIGHT {height} /* {height_source} */
#define S5P6818_FB_BITS_PER_PIXEL {bits_per_pixel} /* {bpp_source} */
#define S5P6818_FB_LINE_LENGTH {line_length} /* {line_length_source} */
#define S5P6818_FB_PIXEL_FORMAT {pixel_format} /* {pixel_format_source} */
#define S5P6818_DISPLAY_ROTATION {rotation} /* -1 means UNKNOWN; {rotation_source} */

#define S5P6818_EVDEV_PATH {input_device} /* {input_device_source} */
#define S5P6818_TOUCH_X_MIN {x_min} /* {x_min_source} */
#define S5P6818_TOUCH_X_MAX {x_max} /* {x_max_source} */
#define S5P6818_TOUCH_Y_MIN {y_min} /* {y_min_source} */
#define S5P6818_TOUCH_Y_MAX {y_max} /* {y_max_source} */
#define S5P6818_TOUCH_SWAP_XY {swap_xy} /* -1 means UNKNOWN; {swap_xy_source} */
#define S5P6818_TOUCH_INVERT_X {invert_x} /* -1 means UNKNOWN; {invert_x_source} */
#define S5P6818_TOUCH_INVERT_Y {invert_y} /* -1 means UNKNOWN; {invert_y_source} */
#define S5P6818_INPUT_EVENT_PROTOCOL {event_protocol} /* {event_protocol_source} */

#if S5P6818_UNVERIFIED_VALUES_PRESENT
/* This header is scaffold data only.  Confirm runtime values before use. */
#endif

#if S5P6818_UNVERIFIED_VALUES_PRESENT && !defined(S5P6818_ACKNOWLEDGE_UNVERIFIED_SCAFFOLD)
#error "UNVERIFIED S5P6818 scaffold: confirm runtime profile values before compilation"
#endif
...
toolchain = """# Generated from s5p6818_profile.json.
# Candidate paths are evidence only; they are never selected automatically.
S5P6818_PROFILE_RUNTIME_CONFIRMED := {runtime_confirmed}
S5P6818_UNVERIFIED_VALUES_PRESENT := {unverified_present}
...
- `s5p6818_profile.json`：生成时的平台证据快照；
...
```sh
KIT_DIR=/path/to/s5p6818_platform_kit
sh "$KIT_DIR/scripts/probe_toolchain.sh"

# 在开发板上只读取系统信息；该脚本不读取触摸事件流。
sh "$KIT_DIR/scripts/probe_s5p6818_board.sh" --output s5p6818_runtime_report.txt
...
> 读取 `s5p6818_platform_kit/s5p6818_profile.json` 和 `README.md`，仅使用与字段匹配的
> `scope=board` 或 `scope=toolchain` 的 `confirmed-runtime` 平台关键值创建 S5P6818 项目。
...
echo "Created standalone S5P6818 LVGL 8 scaffold: $TARGET_FINAL"
if [ "$ALLOW_UNVERIFIED" -eq 1 ]; then
